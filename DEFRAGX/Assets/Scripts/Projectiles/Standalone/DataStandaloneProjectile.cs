...
namespace Projectiles
{
	...
	public sealed class DataStandaloneProjectile : StandaloneProjectile
	{
		...
		{
			if (HasInputAuthority == true && _isActivated == true)
				return; // Spawned (confirmation) called on already active predicted projectile

			// Object.IsProxy is not valid for predicted objects (= true)
			bool isProxy = IsPredicted == false && IsProxy;
		...
		public override void Render()
		{
			...
			bool interpolate = IsPredicted == false && IsProxy == true && _fullProxyPrediction == false;
